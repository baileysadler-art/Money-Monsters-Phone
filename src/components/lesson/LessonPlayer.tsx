...
export default function LessonPlayer({ lessonId }: LessonPlayerProps) {
  ...
  const handleCorrect = () => {
    const xpForQuestion = Math.round(XP_PER_LESSON / lesson.screens.filter(s => s.type !== 'content').length);
  ...
  const renderScreen = () => {
    ...
    switch (quizScreen.type) {
      case 'multiple-choice':
        return <MultipleChoiceScreen key={screenKey} variant={variant} onCorrect={handleCorrect} onWrong={handleWrong} />;
      case 'true-false':
        return <TrueFalseScreen key={screenKey} variant={variant} onCorrect={handleCorrect} onWrong={handleWrong} />;
      case 'tap-match':
        return <TapMatchScreen key={screenKey} variant={variant} onCorrect={handleCorrect} onWrong={handleWrong} />;
      case 'fill-blank':
        return <FillBlankScreen key={screenKey} variant={variant} onCorrect={handleCorrect} onWrong={handleWrong} />;
      default:
        return null;
    }
  };
